/**
 * Thinking mode selector for chat composer controls.
 * Lets users switch provider reasoning behavior without leaving the current chat input.
 */
...
/**
 * Render the compact thinking mode button and dropdown.
 */
function ThinkingModeSelector({ selectedMode, onModeChange, onClose, className = '' }: ThinkingModeSelectorProps) {
  ...
  const translatedModes: ThinkingModeOption[] = thinkingModes.map(mode => {
    const prefix = (mode as Record<string, unknown>).prefix;
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        className={`w-10 h-10 sm:w-10 sm:h-10 rounded-full flex items-center justify-center transition-all duration-200 ${selectedMode === 'disabled'
        ...
        <div className="absolute bottom-full right-0 mb-2 w-64 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="p-3 border-b border-gray-200 dark:border-gray-700">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              {String(t('thinkingMode.selector.description'))}
            </p>
          </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isSelected && (
                          <span className="text-xs bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded">
                            {String(t('thinkingMode.selector.active'))}
                          </span>
                        )}
                      </div>
