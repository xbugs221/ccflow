 ...
 * active oz changes, starting wo runs, and opening ordinary planning sessions.
 */
import { useEffect, useMemo, useState } from 'react';
import { Check, Loader2, MessageSquarePlus, RefreshCw, Square, X } from 'lucide-react';
...
const PLANNING_PROMPT = [
  ...
  '不要启动 wo sealed run，不要创建 .wo/runs/ 运行态目录。',
].join('\n');
...
export default function WorkflowActionDialog({
  ...
  const startPlanningSession = async () => {
     ...
     * not call the workflow creation API and therefore cannot start wo.
     */
      ...
      <div className="flex max-h-[86vh] w-full max-w-2xl flex-col rounded-md border border-border bg-background shadow-lg">
        <div className="flex items-start justify-between gap-4 border-b border-border/60 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">选择 active oz change 后启动对应 wo run，或先发起新的规划会话。</p>
          </div>
          <button
            type="button"
