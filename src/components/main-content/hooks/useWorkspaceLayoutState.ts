...
export type RightDockPanel = 'files' | 'git' | null;
...
function isValidRightDockPanel(value: unknown): value is RightDockPanel {
  return value === 'files' || value === 'git' || value === null;
}
...
function isValidSplit(value: unknown): value is RightDockSplit {
  ...
  return (
    (v.topPanel === 'files' || v.topPanel === 'git')
...
function migrateOldTabState(): Partial<WorkspaceLayoutState> | null {
  try {
    ...
    // If old tab was files/git/shell, migrate to dock layout
    const migrated: Partial<WorkspaceLayoutState> = {};
    if (oldTab === 'files') {
      migrated.rightDock = { ...DEFAULT_STATE.rightDock, activePanel: 'files' };
    } else if (oldTab === 'git') {
      migrated.rightDock = { ...DEFAULT_STATE.rightDock, activePanel: 'git' };
    ...
    setLayout((prev) => {
      ...
      return {
        ...
        rightDock: {
          ...prev.rightDock,
          split: {
            topPanel: prev.rightDock.activePanel as 'files' | 'git',
            bottomPanel: 'terminal',
            ratio: 0.6,
          },
        },
