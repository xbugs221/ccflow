...
import { TASKMASTER_CLI, TASKMASTER_RUNNER, buildTaskMasterRunnerArgs } from '../domains/taskmaster/runner.js';
...
/**
 * Check if the supported TaskMaster CLI is installed globally.
 * @returns {Promise<Object>} Installation status result
 */
async function checkTaskMasterInstallation() {
    return new Promise((resolve) => {
        // Check if the TaskMaster AI executable is available.
        const child = spawn('which', [TASKMASTER_CLI], {
        ...
        child.on('close', (code) => {
            if (code === 0 && output.trim()) {
                // Read CLI version for settings diagnostics.
                const versionChild = spawn(TASKMASTER_CLI, ['--version'], {
                    stdio: ['ignore', 'pipe', 'pipe'],
                    shell: true 
                });
                ...
                resolve({
                    ...
                    installPath: null,
                    version: null,
                    reason: `${TASKMASTER_CLI} not found in PATH`
                });
            }
        });
...
router.post('/initialize/:projectName', async (req, res) => {
    try {
        ...
        res.status(501).json({
            ...
            message: `This endpoint will execute ${TASKMASTER_CLI} init via CLI in a future update`,
            projectName,
            rules
        });
 ...
 * Get the next recommended task using the TaskMaster CLI
 */
router.get('/next/:projectName', async (req, res) => {
    try {
        ...
        // Try to execute the TaskMaster CLI directly.
        try {
            const { spawn } = await import('child_process');
            
            const nextTaskCommand = spawn(TASKMASTER_CLI, ['next'], {
                cwd: projectPath,
            ...
            await new Promise((resolve, reject) => {
                nextTaskCommand.on('close', (code) => {
                    if (code === 0) {
                        ...
                        reject(new Error(`${TASKMASTER_CLI} next failed with code ${code}: ${stderr}`));
                    }
                });
            ...
            // Parse CLI output, preferring structured JSON when available.
            ...
            console.warn(`Failed to execute ${TASKMASTER_CLI}:`, cliError.message);
...
router.post('/init/:projectName', async (req, res) => {
    try {
        ...
        // Run TaskMaster init with pnpm dlx to avoid npm exec overhead.
        const initProcess = spawn(TASKMASTER_RUNNER, buildTaskMasterRunnerArgs('init'), {
            cwd: projectPath,
            stdio: ['pipe', 'pipe', 'pipe']
        });
...
router.post('/add-task/:projectName', async (req, res) => {
    try {
        ...
        // Build the TaskMaster AI add-task command.
        const args = buildTaskMasterRunnerArgs('add-task');
        ...
        // Run the TaskMaster AI add-task command.
        const addTaskProcess = spawn(TASKMASTER_RUNNER, args, {
            cwd: projectPath,
            stdio: ['pipe', 'pipe', 'pipe']
        });
...
router.put('/update-task/:projectName/:taskId', async (req, res) => {
    try {
        ...
        if (status && Object.keys(req.body).length === 1) {
            const setStatusProcess = spawn(TASKMASTER_RUNNER, buildTaskMasterRunnerArgs('set-status', `--id=${taskId}`, `--status=${status}`), {
                cwd: projectPath,
                stdio: ['pipe', 'pipe', 'pipe']
            });
            ...
            const updateProcess = spawn(TASKMASTER_RUNNER, buildTaskMasterRunnerArgs('update-task', `--id=${taskId}`, `--prompt=${prompt}`), {
                cwd: projectPath,
                stdio: ['pipe', 'pipe', 'pipe']
            });
...
router.post('/parse-prd/:projectName', async (req, res) => {
    try {
        ...
        const args = buildTaskMasterRunnerArgs('parse-prd', prdPath);
        ...
        const parsePRDProcess = spawn(TASKMASTER_RUNNER, args, {
            cwd: projectPath,
            stdio: ['pipe', 'pipe', 'pipe']
        });
