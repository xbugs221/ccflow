...
WebUI 中“改名项目”在部分项目类型（尤其 Codex-only 项目）刷新后不生效，导致用户无法稳定识别项目。与此同时，输入区 mode 按钮旁的百分比仅反映上下文 token 占用，对真实配额决策帮助有限，用户更需要与 CLI statusline 对齐的“5 小时/7 天剩余额度”信息。

## What Changes

- 修复项目改名持久化链路，确保改名后刷新仍生效。
...
### New Capabilities
- `project-display-name-persistence`: 项目展示名在改名后可跨刷新、跨项目来源稳定生效。
