...
Define the behavior for automatically archiving stale projects whose underlying paths no longer exist, so the active project sidebar only shows usable projects while preserving historical session data.
