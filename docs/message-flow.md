...
> 目标：统一 Codex、OpenCode 及后续助手在前端展示的消息格式，
> 确保实时推送、历史文件、页面刷新三者所见一致。

## 目标

把 Codex、OpenCode 以及后续其它助手接到同一条聊天链路上。网页端看到的消息、后端推送的消息、历史文件重新加载出来的消息必须一致。
...
- **助手适配层（Adapter）**：负责把 Codex、OpenCode 的原始输出转成统一消息事件，申请消息编号，管理历史缓冲区，处理 steer 注入。
...
sequenceDiagram
    ...
    Chat->>Adapter: 选择 Codex 或 OpenCode
    Adapter->>Agent: 启动助手
...
Codex 和 OpenCode 支持在助手运行中插入用户消息，无需中断。steer 消息在当前输出边界（tool call 结束或 chunk 边界）注入上下文，助手继续生成。
...
1. **后端增加统一消息整理层。**
   Codex、OpenCode 原始输出不能直接推给前端。先转成统一消息，再推送。
...
4. **所有助手完成后都触发同一种历史校准。**
   Codex 和 OpenCode 都在本轮完成后发 `历史已变化`，前端统一重新读取本会话历史。

5. **历史读取层输出统一格式。**
   前端不应该知道 Codex 历史文件和 OpenCode 历史文件结构差异。
...
- Codex、OpenCode 的前端处理代码没有两套拼消息逻辑。
