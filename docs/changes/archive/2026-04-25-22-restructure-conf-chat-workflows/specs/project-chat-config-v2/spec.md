...
### Requirement: 旧会话配置必须迁移为 v2 结构

系统 MUST 能读取旧版平铺字段，并在下一次保存时输出等价的 v2 `chat` / `workflows` 结构。

#### Scenario: 旧字段迁移为 chat 记录

- **WHEN** 旧配置包含 `sessionRouteIndex`、`sessionSummaryById`、`sessionModelStateById` 和 `sessionUiStateByPath`
- **AND** 这些字段描述同一个 standalone 会话
- **THEN** 系统读取后生成对应的 `chat["<编号>"]`
- **AND** `sessionId`、`title`、`model`、`reasoningEffort` 和 `ui` 都保留

#### Scenario: 旧工作流字段迁移为 workflows 记录

- **WHEN** 旧配置通过 `sessionWorkflowMetadataById` 标记工作流内部会话
- **THEN** 系统读取后将这些会话放入对应 `workflows["<编号>"].chat`
- **AND** 工作流内部会话不会出现在顶层 `chat`
- **AND** 工作流编号由旧 workflow 归属稳定映射为数字 key
