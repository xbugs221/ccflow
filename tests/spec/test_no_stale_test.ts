...
describe('no-stale-test-contract', () => {
  it('测试文件不再引用已删除的 .jsx 入口', () => {
    const jsxRefs = execSync(
      `rg -l 'src/main\\.jsx|SetupForm\\.jsx|Onboarding\\.jsx|ProjectCreationWizard\\.jsx|LanguageSelector\\.jsx' "${TESTS_DIR}" 2>/dev/null || true`,
      { encoding: 'utf8' },
    ).trim();
  ...
  it('e2e 测试代码不硬编码 .wo/runs 或 .cbw/runs 作为真实文件路径', () => {
    // Allow .wo/runs in JSON fixture data (display paths per spec.md),
    // but files must use resolveWoRunStatePath/resolveWoRunsRoot for real path operations.
    const raw = execSync(
      `rg -n '\\.wo/runs|\\.cbw/runs' "${TESTS_DIR}/e2e" 2>/dev/null || true`,
      { encoding: 'utf8' },
    ).trim();
    ...
    const suspicious = raw.split('\n').filter((line) => {
      ...
      if (line.includes("'.wo/runs") || line.includes('\".wo/runs') ||
          line.includes("'.cbw/runs") || line.includes('\".cbw/runs')) {
        return false;
      }
      return true;
    });
