...
test.describe('需求工作流详情路由与会话流程图预览', () => {
  ...
  test('侧边栏按需求工作流与手动会话分组显示', async ({ page }) => {
    ...
    await expect(page.getByTestId('manual-session-group')).toContainText('fixture-project session');
  });
  ...
  test('工作流详情展示正文流程图，工作流会话展示只读流程图预览', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/workspace\/fixture-project\/runs\/run-fixture\/sessions\/planning$/);
