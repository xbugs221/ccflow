...
test('OpenCode backend modules exist', async () => {
  assert.ok(await fileExists('server/opencode-sdk.ts'), 'server/opencode-sdk.ts must exist');
  assert.ok(await fileExists('server/routes/opencode.ts'), 'server/routes/opencode.ts must exist');
});
...
test('transformOpencodeEvent maps step_start to turn_started', async () => {
  const { __transformOpencodeEventForTest } = await import('../../server/opencode-sdk.ts');
...
test('enqueueSteer adds messages to FIFO queue', async () => {
  const { enqueueSteer, getSteerQueue, clearSteerQueue } = await import('../../server/opencode-sdk.ts');
...
test('buildOpencodeExecArgs for new session', async () => {
  const { __buildOpencodeExecArgsForTest } = await import('../../server/opencode-sdk.ts');
...
test('queryOpencode reports real CLI failures by throwing after opencode-error', async () => {
  ...
  try {
    ...
    const { queryOpencode } = await import(`../../server/opencode-sdk.ts?failure=${Date.now()}`);
    const sent = [];
    const writer = {
...
test('OpenCode REST routes handle models, sessions, messages, and delete', async () => {
  const source = await readRepoFile('server/routes/opencode.ts');
...
test('server/index.js routes OpenCode chat through co client functions', async () => {
  const source = await readRepoFile('server/index.ts');
...
test('server/projects.js normalizes opencode provider in chat records', async () => {
  const source = await readRepoFile('server/projects.ts');
...
test('Playwright fake co enforces duplicate request and stale abort semantics', async () => {
  const source = await readRepoFile('tests/e2e/helpers/playwright-global-setup.ts');
