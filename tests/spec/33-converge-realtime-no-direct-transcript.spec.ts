...
async function installFakeWebSocket(page, cfg: ProviderConfig) {
   ...
   * check-session-status and exposes window.__cbwTriggerReload() so the
   ...
   * To keep each test correct, __cbwTriggerReload() resolves the provider
  ...
  await page.addInitScript(() => {
      ...
      send(payload: string) {
        ...
        if (message.type === 'check-session-status') {
          ...
          for (const event of [
            realtimeNoise,
            realtimeNoise,
            { type: 'session-status', sessionId: cid, cbwSessionId: cid, isProcessing: false },
          ]) {
    ...
    (window as any).__cbwTriggerReload = () => {
