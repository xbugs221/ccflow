...
test.describe('项目规范路由寻址', () => {
  ...
  test('新建手动会话后无需刷新即可进入会话路由', async ({ page }) => {
    ...
    await page.getByTestId('project-overview-manual-sessions').getByRole('button', { name: /新建会话/i }).click();
  ...
  test('工作流子会话使用嵌套的 w1/c1 路由', async ({ page }) => {
    const projectRoutePrefix = buildExpectedProjectRoutePrefix();
