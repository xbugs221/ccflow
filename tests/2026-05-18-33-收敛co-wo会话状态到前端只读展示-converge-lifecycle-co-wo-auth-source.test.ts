...
describe('前端不复制 co/wo 生命周期', () => {
  it('useChatComposerState 不再直接 setProcessingStatus', () => {
    const source = readSource('src/components/chat/hooks/useChatComposerState.ts');
  ...
  it('useChatSessionState 不再用 processingSessions 反推 isLoading', () => {
    const source = readSource('src/components/chat/hooks/useChatSessionState.ts');
  ...
  it('session-status 事件仍控制 isLoading 和 canAbortSession', () => {
    const source = readSource('src/components/chat/hooks/useChatRealtimeHandlers.ts');
  ...
  it('ChatComposer 停止按钮仍可用（isLoading true 时显示 stop）', () => {
    const source = readSource('src/components/chat/view/subcomponents/ChatComposer.tsx');
...
describe('useSessionProtection 降级', () => {
  it('useSessionProtection 不再维护 processingSessions Set', () => {
    const source = readSource('src/hooks/useSessionProtection.ts');
