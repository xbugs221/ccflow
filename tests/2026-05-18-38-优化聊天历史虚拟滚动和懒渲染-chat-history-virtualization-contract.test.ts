...
} from '../src/components/chat/utils/transcriptVirtualization.ts';
...
test('chat transcript implementation uses measured continuous virtual ranges', () => {
  ...
  const source = readSource('src/components/chat/view/subcomponents/ChatMessagesPane.tsx');
...
test('search navigation pages toward unloaded targets without using load-all', () => {
  ...
  const sessionState = readSource('src/components/chat/hooks/useChatSessionState.ts');
  const chatInterface = readSource('src/components/chat/view/ChatInterface.tsx');
...
test('collapsed tool sections do not mount heavy children until expanded', () => {
  ...
  const source = readSource('src/components/chat/tools/components/CollapsibleSection.tsx');
...
test('heavy markdown, diff, tool output, and subagent timelines have lazy summaries', () => {
  ...
  const markdown = readSource('src/components/chat/view/subcomponents/Markdown.tsx');
  const diffViewer = readSource('src/components/chat/tools/components/DiffViewer.tsx');
  const textContent = readSource('src/components/chat/tools/components/ContentRenderers/TextContent.tsx');
  const subagent = readSource('src/components/chat/tools/components/SubagentContainer.tsx');
