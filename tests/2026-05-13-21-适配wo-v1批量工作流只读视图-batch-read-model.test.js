...
import {
  ...
  buildWoWorkflowReadModel,
  listBatchReadModels,
  listWoWorkflowReadModels,
} from '../server/domains/workflows/wo-read-model.js';
import { resolveWoBatchesRoot, resolveWoRunsRoot } from '../server/domains/workflows/wo-runtime-paths.js';
...
test('opencode provider links while pi provider remains unlinked', async () => {
  await withTestEnv(async ({ projectPath }) => {
    const runsRoot = resolveWoRunsRoot(projectPath);
    ...
    assert.equal(executor.sessionRef.unlinked, true);

    const reviewer = rows.find((r) => r.key === 'reviewer');
    assert.equal(reviewer.sessionRef.sessionId, 'codex-reviewer-session');

    const archiver = rows.find((r) => r.key === 'archiver');
    assert.equal(archiver.sessionRef.sessionId, 'opencode-archiver-session');
    assert.equal(archiver.sessionRef.unlinked, undefined);
  });
});
  ...
  await withTestEnv(async ({ projectPath }) => {
    ...
    await fs.writeFile(path.join(runDir, 'state.json'), JSON.stringify({
      ...
      sessions: {
        ...
        'opencode:reviewer': 'opencode-reviewer-session',
      },
    }));
    ...
    assert.deepEqual(diagnostics.workflowOwnedSessions, [
      ...
      {
        key: 'opencode:reviewer',
        role: 'reviewer',
        provider: 'opencode',
        sessionId: 'opencode-reviewer-session',
      },
    ]);
  });
});

test('opencode provider sessions from wo state sessions are filtered from backend manual list', async () => {
  await withTestEnv(async ({ projectPath }) => {
    const runsRoot = resolveWoRunsRoot(projectPath);
    ...
    const indexRef = {
      sessionsByProject: new Map([[
        ...
        [
          ...
          {
            id: 'opencode-manual-session',
            name: 'manual session',
            lastActivity: '2026-05-13T00:01:00.000Z',
    ...
    assert.deepEqual(sessions.map((session) => session.id), ['opencode-manual-session']);
  });
});

test('unknown provider generates unlinked session reference', async () => {
  await withTestEnv(async ({ projectPath }) => {
    const runsRoot = resolveWoRunsRoot(projectPath);
    ...
    assert.equal(executorRow.sessionRef.unlinked, true);
    assert.equal(executorRow.sessionRef.provider, 'unknown');
    assert.equal(executorRow.sessionRef.sessionId, 'unknown-executor-session');
  });
});
