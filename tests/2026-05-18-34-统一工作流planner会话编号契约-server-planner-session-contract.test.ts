...
/**
 * PURPOSE: Verify cbw read model follows the current wo planner session
...
async function withTempProject(callback) {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'cbw-planner-contract-'));
...
test('codex:planner 规划会话可链接到工作流子会话', async () => {
  await withTempProject(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-planner');
...
test('pi:planner 非 codex planner 可正确识别 provider', async () => {
  await withTempProject(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-pi-planner');
    await fs.mkdir(runRoot, { recursive: true });
    await fs.writeFile(path.join(runRoot, 'state.json'), JSON.stringify({
...
test('规划会话缺失时显示未知占位符', async () => {
  await withTempProject(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-no-planner');
    await fs.mkdir(runRoot, { recursive: true });
    await fs.writeFile(path.join(runRoot, 'state.json'), JSON.stringify({
...
test('sessions-only 状态不产生 runnerProcesses 但 childSessions 仍从 role map 构建', async () => {
  await withTempProject(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-sessions-only');
    await fs.mkdir(runRoot, { recursive: true });
    await fs.writeFile(path.join(runRoot, 'state.json'), JSON.stringify({
...
test('explicit processes 保留 pid 和 sessionId 且二者不混淆', async () => {
  await withTempProject(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-with-processes');
    await fs.mkdir(runRoot, { recursive: true });
    await fs.writeFile(path.join(runRoot, 'state.json'), JSON.stringify({
...
test('旧 sessions fallback 不再合成 process rows', async () => {
  await withTempProject(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-old-fallback');
    await fs.mkdir(runRoot, { recursive: true });
    await fs.writeFile(path.join(runRoot, 'state.json'), JSON.stringify({
