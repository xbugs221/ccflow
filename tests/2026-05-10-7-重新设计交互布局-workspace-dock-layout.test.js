...
test.describe('mobile workspace layout', () => {
  ...
  test('mobile switches between files, source control, terminal, and chat as single views', async ({ page }) => {
    await openTestProject(page);
    ...
    await clickTab(page, 'git');
    await expect(page.locator('[data-testid="mobile-workspace-git"]')).toBeVisible();
    await expect(page.locator('[data-testid="tab-git"]')).toHaveAttribute('aria-pressed', 'true');
    await expect(page.locator('[data-testid="mobile-workspace-files"]')).toHaveCount(0);
    await expect(page.locator('[data-testid="mobile-overlay-close"]')).toHaveCount(0);
