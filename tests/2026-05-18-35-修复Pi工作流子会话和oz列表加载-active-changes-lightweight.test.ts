...
import { listProjectAdoptableOpenSpecChanges } from '../server/workflows.ts';
...
async function withFakePath(callback) {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'cbw-active-changes-'));
...
test('listAdoptableOpenSpecChanges returns unclaimed active changes', async () => {
  await withFakePath(async ({ binDir, projectPath }) => {
    ...
    // Create a wo run that has claimed 'change-a' in the wo user-state tree
