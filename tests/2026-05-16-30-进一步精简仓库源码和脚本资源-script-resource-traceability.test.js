...
function grepAll(query, searchDirs = 'package.json README.md index.html src/ server/ tests/') {
  try {
    execSync(
...
test('all public/ files have traceable references from runtime entry points', async () => {
  ...
  const runtimeDirs = 'package.json README.md index.html src/ server/';
...
test('fix-node-pty.js is referenced in package.json', async () => {
  const pkg = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, 'package.json'), 'utf8'));
  const hasFix = Object.values(pkg.scripts || {}).some(
    (s) => typeof s === 'string' && s.includes('fix-node-pty.js'),
  );
  assert.ok(hasFix, 'fix-node-pty.js not referenced in package.json');
});

test('verify-missing-session-visibility.mjs is referenced', async () => {
  const pkg = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, 'package.json'), 'utf8'));
