...
describe('ProcessingStatus 底部状态条已删除', () => {
  it('ProcessingStatus.tsx 文件已删除', () => {
    assert.equal(
      existsSync(path.join(REPO_ROOT, 'src/components/chat/view/subcomponents/ProcessingStatus.tsx')),
      false,
  ...
  it('ChatInterface 不再传递 processingStatus 给 ChatComposer', () => {
    const source = readSource('src/components/chat/view/ChatInterface.tsx');
    assert.doesNotMatch(source, /processingStatus/);
  });
