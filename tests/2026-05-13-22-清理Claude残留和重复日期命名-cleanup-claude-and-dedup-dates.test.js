...
test('server/claude-sdk.js does not exist', async () => {
  assert.equal(await fileExists('server/claude-sdk.js'), false, 'server/claude-sdk.js must be deleted');
});

test('no production code references claude-sdk', () => {
  try {
    const result = execFileSync(
      'rg', ['-l', "claude-sdk", 'server/', 'src/', 'shared/'],
...
test('tests/server/claude-sdk.unsupported.test.js does not exist', async () => {
  assert.equal(
    await fileExists('tests/server/claude-sdk.unsupported.test.js'),
    false,
...
test('no claudeStatus state variable remains in frontend chat code', async () => {
  try {
    execFileSync(
      'rg', ['-l', '-e', 'claudeStatus', '-e', 'setClaudeStatus', 'src/components/chat/'],
...
test('playwright.spec.config.js does not reference old dup date paths', async () => {
  const config = await readSource('playwright.spec.config.js');
