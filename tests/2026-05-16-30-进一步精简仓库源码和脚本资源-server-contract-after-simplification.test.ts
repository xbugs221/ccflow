...
test('server/projects.js imports from shared/codex-message-normalizer.ts', async () => {
  const content = fs.readFileSync(path.join(REPO_ROOT, 'server/projects.ts'), 'utf8');
  assert.ok(
...
test('server/codex-models.js imports from shared/modelConstants.ts', async () => {
  const content = fs.readFileSync(path.join(REPO_ROOT, 'server/codex-models.ts'), 'utf8');
  assert.ok(
...
test('server/executable-resolver.js exports resolveExecutablePath', async () => {
  const content = fs.readFileSync(path.join(REPO_ROOT, 'server/executable-resolver.ts'), 'utf8');
...
test('runtime-dependencies imports from executable-resolver', async () => {
  const content = fs.readFileSync(path.join(REPO_ROOT, 'server/runtime-dependencies.ts'), 'utf8');
...
test('commandParser.js BASH_COMMAND_ALLOWLIST excludes task-master', async () => {
  const content = fs.readFileSync(path.join(REPO_ROOT, 'server/utils/commandParser.ts'), 'utf8');
  assert.ok(!content.includes("'task-master'"), 'task-master still present');
});

test('MCP routes file returns 410 for all requests', async () => {
  const content = fs.readFileSync(path.join(REPO_ROOT, 'server/routes/mcp.ts'), 'utf8');
...
test('server/index.js wires MCP routes for 410 rejection', async () => {
  const content = fs.readFileSync(path.join(REPO_ROOT, 'server/index.ts'), 'utf8');
...
test('workflow read model exports build and list functions', async () => {
  const mod = await import(serverMod('domains', 'workflows', 'wo-read-model.ts'));
...
test('Git API routes file exports an express router', async () => {
  const mod = await import(serverMod('routes', 'git.ts'));
