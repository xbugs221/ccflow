...
// ═══════════════════════════════════════════════════════════════════
// Git panel — component and server route
// ═══════════════════════════════════════════════════════════════════

test('Git panel component exists and exports default', () => {
  const f = sourceFile('src', 'components', 'git-panel', 'view', 'GitPanel.tsx');
  assert.ok(sourceExists(f), 'GitPanel component must exist');
  const content = readSource(f);
  assert.ok(hasDefaultExport(content), 'Must export a default component');
});

test('Git API server route is wired and exportable', async () => {
  const indexContent = readSource(sourceFile('server', 'index.ts'));
  assert.ok(indexContent.includes('/api/git'), 'Server must wire /api/git');
  // The router module can be imported
  const mod = await import(path.join(REPO_ROOT, 'server', 'routes', 'git.ts'));
  assert.ok(typeof mod.default === 'function' || typeof mod.default?.use === 'function',
    'Git router must be a valid Express router');
});
...
test('message dedup TS modules exist and export dedup functions', () => {
  const dedupFiles = [
    { path: 'src/components/chat/utils/messageDedup.ts', expected: 'dedupeAdjacentChatMessages' },
    { path: 'src/components/chat/utils/sessionMessageDedup.ts', expected: 'dedupe' },
  ];
