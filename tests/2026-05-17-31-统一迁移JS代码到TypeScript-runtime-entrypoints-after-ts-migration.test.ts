 ...
 *
 * PURPOSE: Verify the cbw bin, compiled CLI, and dev scripts actually execute
...
describe('runtime-entrypoints-after-ts-migration', () => {
  it('compiled CLI help prints usage instructions', async () => {
    const { stdout, stderr } = await execFileAsync('node', ['dist-node/server/cli.js', 'help'], {
      cwd: process.cwd(),
      timeout: 15000,
    });
    assert.ok(
      stdout.includes('cbw - Command Line Tool') && stdout.includes('start') && stdout.includes('status'),
  ...
  it('compiled bin CLI has node shebang (package bin is Node-runnable)', () => {
    const cliContent = readFileSync('dist-node/server/cli.js', 'utf8');
    assert.ok(
      cliContent.startsWith('#!/usr/bin/env node'),
      'dist-node/server/cli.js should start with node shebang',
    );
  });

  it('compiled CLI version outputs a non-empty semver', async () => {
    const { stdout, stderr } = await execFileAsync('node', ['dist-node/server/cli.js', 'version'], {
      cwd: process.cwd(),
      timeout: 15000,
    });
  ...
  it('compiled CLI status reports installation directory', async () => {
    const { stdout, stderr } = await execFileAsync('node', ['dist-node/server/cli.js', 'status'], {
      cwd: process.cwd(),
      timeout: 15000,
    });
    assert.ok(
  ...
  it('compiled CLI start on a free port boots to Ready and responds to SIGTERM', async () => {
    ...
    const dbPath = `/tmp/cbw-test-smoke-${port}.db`;

    const child = spawn('node', ['dist-node/server/cli.js', 'start', '--port', String(port), '--database-path', dbPath], {
      cwd: process.cwd(),
    ...
    child.stdout.on('data', (chunk: Buffer) => {
      ...
      if (stdout.includes('cbw Server - Ready')) {
        readySeen = true;
        child.kill('SIGTERM');
      }
    });
    ...
    assert.ok(readySeen, `Server should print 'cbw Server - Ready'. stdout: ${stdout.slice(0, 500)}`);
    assert.ok(
  ...
  it('dev:watch script uses tsx watch instead of raw node .js', () => {
    ...
    assert.ok(
      ...
      'dev-watch.sh should use tsx server/index.ts, not node server/index.js',
    );
    assert.ok(
      !/node\s+.*server\/index\.js/.test(sh),
      'dev-watch.sh must not reference deleted server/index.js',
    );
  });
  ...
  it('package.json main and bin point to dist-node compiled JS, not raw TS', () => {
    ...
    const cbwBin = String(pkg.bin?.cbw || '');
    assert.ok(
      cbwBin.endsWith('.js') && cbwBin.startsWith('dist-node/'),
      `"bin.cbw" should point to dist-node/ .js, got: ${cbwBin}`,
    );
    assert.ok(
      !cbwBin.endsWith('.ts'),
      `"bin.cbw" must not point to raw .ts, got: ${cbwBin}`,
    );
  });
});
