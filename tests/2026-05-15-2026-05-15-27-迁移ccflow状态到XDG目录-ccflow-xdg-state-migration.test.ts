...
} from '../server/project-config-store.ts';
...
// ─── Old project config migration ─────────────────────────────────────────

describe('old project config migration', () => {
  let env;
  ...
  test('reads old .cbw/conf.json and migrates to state config', async () => {
    ...
    try {
      ...
      assert.deepEqual(config.sessions.c1, oldConfig.sessions.c1);
      assert.deepEqual(config.sessions.c2, oldConfig.sessions.c2);

      // State config should now exist
      const repoKey = resolveProjectStateKey(projectPath);
      const stateConfigPath = join(resolveCcflowStateRoot({ XDG_STATE_HOME: env.stateHome }), 'repos', repoKey, 'conf.json');
      const raw = await readFile(stateConfigPath, 'utf8');
      const parsed = JSON.parse(raw);
      assert.deepEqual(parsed.sessions.c1, oldConfig.sessions.c1);
      assert.deepEqual(parsed.sessions.c2, oldConfig.sessions.c2);
    } finally {
      await rm(projectPath, { recursive: true, force: true });
    }
  });
  ...
  test('migrated config preserves session route metadata', async () => {
    ...
    try {
      const oldConfig = {
        sessions: {
          c1: { route: '/c1', model: 'gpt-5' },
          c2: { route: '/c2', providerSessionId: 'prov-sess-2' },
        },
      };
      await createOldProjectConfig(projectPath, oldConfig);

      const config = await readProjectLocalConfig(projectPath);
      assert.equal(config.sessions?.c1?.route, '/c1');
      assert.equal(config.sessions?.c1?.model, 'gpt-5');
      assert.equal(config.sessions?.c2?.route, '/c2');
      assert.equal(config.sessions?.c2?.providerSessionId, 'prov-sess-2');
    } finally {
      await rm(projectPath, { recursive: true, force: true });
    }
  });
  ...
  test('concurrent first-readers all succeed without temp file collisions', async () => {
    ...
    try {
      ...
      // Run 20 concurrent first-time reads — all should succeed without
      // temp-file collisions or ENOENT renames.
      ...
      // State config must contain the migrated data exactly once
      const repoKey = resolveProjectStateKey(projectPath);
...
// ─── Old global config migration ──────────────────────────────────────────

describe('old global config migration', () => {
  ...
  test('reads old ~/.cbw/conf.json and migrates to state root', async () => {
    ...
    await createOldGlobalConfig(env.stateHome, oldGlobalConfig);

    const config = await readProjectLocalConfig('');
    assert.deepEqual(config, oldGlobalConfig);

    // State root config should now exist
    const stateConfigPath = join(resolveCcflowStateRoot({ XDG_STATE_HOME: env.stateHome }), 'conf.json');
    const raw = await readFile(stateConfigPath, 'utf8');
    const parsed = JSON.parse(raw);
    assert.deepEqual(parsed, oldGlobalConfig);
  });
});
