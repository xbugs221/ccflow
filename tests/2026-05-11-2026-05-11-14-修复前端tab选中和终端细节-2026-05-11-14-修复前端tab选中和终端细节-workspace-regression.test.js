...
test('desktop dock buttons keep chat visible while opening files terminal and git', async ({ page }) => {
  await openFixtureSession(page);
...
test('file dock shows one file title and keeps file actions available', async ({ page }) => {
  ...
  await expect(rightDock.getByRole('button', { name: /Add File|新建文件/ })).toBeVisible();
  await expect(rightDock.getByRole('button', { name: /Add Folder|新建文件夹/ })).toBeVisible();
  await expect(rightDock.getByRole('button', { name: /Upload Files|上传文件/ })).toBeVisible();
