...
test('Deleted CodexLogo component no longer exists', async () => {
  assert.equal(await exists('src/components/llm-logo-provider/CodexLogo.tsx'), false);
});

test('Deleted ClaudeLogo component no longer exists', async () => {
  assert.equal(await exists('src/components/llm-logo-provider/ClaudeLogo.tsx'), false);
});
