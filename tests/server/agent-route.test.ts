...
} = await import('../../server/routes/agent.ts');
...
test('cleanupProject removes clones under .cbw external-projects', async () => {
  ...
  assert.equal(isCcflowExternalProjectPath(projectDir), true);
...
test('cleanupProject refuses external-projects root and unrelated paths', async () => {
  ...
  assert.equal(isCcflowExternalProjectPath(externalProjectsRoot), false);
  assert.equal(isCcflowExternalProjectPath(unrelatedProject), false);
