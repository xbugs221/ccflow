...
import { listOpenSpecChanges } from '../../server/domains/openspec/oz-client.ts';
import { startGoWorkflowRun } from '../../server/domains/workflows/go-runner-client.ts';
import { listWoWorkflowReadModels } from '../../server/domains/workflows/wo-read-model.ts';
...
test('oz list and wo run use the new command names and wo user-state run root', async () => {
  await withFakePath(async ({ binDir, projectPath }) => {
    const fakeRunsRoot = resolveWoRunsRoot(projectPath);
...
test('wo read model emits only happened display lines and session warnings', async () => {
  await withFakePath(async ({ projectPath }) => {
    const runRoot = resolveWoRunsRoot(projectPath);
...
test('wo read model ignores legacy project .wo runs when user-state runs exist', async () => {
  await withFakePath(async ({ projectPath }) => {
    ...
    const userStateRunRoot = path.join(resolveWoRunsRoot(projectPath), 'new-run');
...
test('wo read model returns empty when only legacy project .wo runs exist', async () => {
  await withFakePath(async ({ projectPath }) => {
    ...
    assert.deepEqual(await listWoWorkflowReadModels(projectPath), []);
  });
});

test('wo read model sorts arbitrary review and repair rounds without unknown warnings', async () => {
  await withFakePath(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-many-rounds');
...
test('wo read model keeps workflow display text from state lines', async () => {
  await withFakePath(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-display-priority');
    await fs.mkdir(runRoot, { recursive: true });
    await fs.writeFile(path.join(runRoot, 'state.json'), JSON.stringify({
...
test('wo read model treats current fix stages as repair rounds', async () => {
  await withFakePath(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-fix-rounds');
    await fs.mkdir(runRoot, { recursive: true });
    await fs.writeFile(path.join(runRoot, 'state.json'), JSON.stringify({
...
test('wo read model leaves unknown terminal state rows non-clickable', async () => {
  await withFakePath(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-blocked-after-fix');
    await fs.mkdir(runRoot, { recursive: true });
    await fs.writeFile(path.join(runRoot, 'state.json'), JSON.stringify({
...
test('wo read model links role jsonl checklist rows to frontend sessions', async () => {
  await withFakePath(async ({ projectPath }) => {
    const runRoot = path.join(resolveWoRunsRoot(projectPath), 'run-role-sessions');
    await fs.mkdir(runRoot, { recursive: true });
