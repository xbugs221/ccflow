 ...
 * utility (src/utils/session-provider.ts) so the test verifies the actual
...
import { resolveSessionProvider } from '../../src/utils/session-provider.ts';
...
test('sessions-only Pi executor resolves to __provider=pi via execution address', async () => {
  const projectPath = path.join(os.tmpdir(), `cbw-route-res-pi-${Date.now()}`);
...
test('multi-provider execution: codex claims address, pi uses by-id with __provider=pi', async () => {
  const projectPath = path.join(os.tmpdir(), `cbw-route-res-multi-${Date.now()}`);
...
test('explicit process + sessions-only: codex owns execution, pi at by-id with __provider=pi', async () => {
  const projectPath = path.join(os.tmpdir(), `cbw-route-res-explicit-${Date.now()}`);
...
test('refresh/reload: same route address always resolves to same provider', async () => {
  const projectPath = path.join(os.tmpdir(), `cbw-route-refresh-${Date.now()}`);
  ...
  try {
    ...
    const model1 = await buildWoWorkflowReadModel({
      projectPath, runDirName: 'run-refresh', state: stateObj, statePath, stateStat: stat,
    });
    const model2 = await buildWoWorkflowReadModel({
      projectPath, runDirName: 'run-refresh', state: stateObj, statePath, stateStat: stat,
    });
