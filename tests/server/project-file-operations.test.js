 ...
 * read-only inspection of wo runtime artifacts owned by the selected project.
 */
...
import { resolveReadableProjectPath } from '../../server/project-file-operations.js';
...
test('readable path resolution allows same-project wo runtime artifacts as read-only files', async () => {
  await withRuntimeStateHome(async (tempRoot) => {
    const projectPath = path.join(tempRoot, 'project');
    const runDir = path.join(resolveWoRunsRoot(projectPath), 'run-a');
